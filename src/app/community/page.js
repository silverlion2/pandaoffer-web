...
const EVENTS = [
  {
    id: 1,
    title: 'Shanghai Tech Alumni Mixer',
    date: 'Friday, Oct 24 • 7:00 PM',
    location: 'Founders Space, Shanghai',
    attendees: 42,
    type: 'Networking'
  },
  {
    id: 2,
    title: 'HSK 5/6 Study Group',
    date: 'Every Saturday • 2:00 PM',
    location: 'Wudaokou Cafe, Beijing',
    attendees: 15,
    type: 'Study'
  }
];
...
const GROUPS = [
  ...
  {
    name: 'International Med Students',
    platform: 'Discord',
    members: '1.2k',
    status: 'Active',
...
export default function CommunityBoard() {
  ...
  const cities = ['All', 'Shanghai', 'Beijing', 'Shenzhen', 'Guangzhou'];

  const filteredEvents = EVENTS.filter(e => 
    cityFilter === 'All' || e.location.includes(cityFilter)
  );
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="flex items-center justify-between">
              ...
              <Link href="#" className="font-bold text-sm text-indigo-600 hover:text-indigo-700">View All</Link>
            </div>

            <div className="space-y-4">
              {filteredEvents.length === 0 ? (
                <div className="text-center py-8 bg-white rounded-2xl border border-slate-200 border-dashed">
                  <p className="text-slate-500 font-medium">No events currently scheduled in {cityFilter}.</p>
